 ...
 * Uses API key from environment variables (EXPO_PUBLIC_OPENWEATHERMAP_API_KEY)
 */
async function fetchOpenWeatherData(
  ...
  try {
    ...
    const apiKey = apiKeyOverride || WEATHER_CONFIG.apiKey;
    
    if (!apiKey) {
      console.error('❌ OpenWeatherMap API key not configured');
      console.error('   Add EXPO_PUBLIC_OPENWEATHERMAP_API_KEY to your .env file');
      throw new Error('OpenWeatherMap API key not configured. Add EXPO_PUBLIC_OPENWEATHERMAP_API_KEY to your .env file.');
    }
