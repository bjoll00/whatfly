-- COMPREHENSIVE FLY DATABASE MIGRATION
-- Expands the flies table to support the hierarchical fly suggestion algorithm
-- Adds all missing fields and populates with detailed fly information

-- First, let's add the missing columns to the flies table
ALTER TABLE flies 
...
-- Update the best_conditions column to include missing fields
-- We'll do this by updating existing flies with comprehensive condition data

-- Create a comprehensive fly database with all the detailed information
-- This will replace the current basic flies with sophisticated, algorithm-ready data

-- Clear existing flies (we'll replace them with comprehensive data)
DELETE FROM flies;

-- Insert comprehensive fly database with all hierarchical algorithm requirements
INSERT INTO flies (
  id, name, type, pattern_name, sizes_available, primary_size, color, secondary_colors,
...
-- Update the confidence scores based on the hierarchical algorithm
...
CREATE INDEX IF NOT EXISTS idx_flies_primary_size ON flies(primary_size);
...
COMMENT ON TABLE flies IS 'Comprehensive fly database with detailed attributes for hierarchical fly suggestion algorithm';
COMMENT ON COLUMN flies.regional_effectiveness IS 'JSON object containing regional effectiveness data';
COMMENT ON COLUMN flies.hatch_matching IS 'JSON object containing hatch matching information';
COMMENT ON COLUMN flies.characteristics IS 'JSON object containing fly physical characteristics';
COMMENT ON COLUMN flies.target_species IS 'JSON object containing target species information';
COMMENT ON COLUMN flies.best_conditions IS 'Enhanced JSON object with comprehensive condition matching data';

-- Verify the migration
SELECT 
  COUNT(*) as total_flies,
  COUNT(DISTINCT type) as fly_types,
  COUNT(DISTINCT primary_size) as size_variations,
